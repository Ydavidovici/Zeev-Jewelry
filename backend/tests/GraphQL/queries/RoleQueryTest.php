...
class RoleQueryTest extends TestCase
{
    ...
    {
        parent::setUp();
        // Set up necessary data, e.g., create a test user and role
        $this->user = User::factory()->create();
        $this->role = Role::factory()->create();
    }

    /** @test */
    public function it_returns_role_data()
    {
    ...
    /** @test */
    public function it_fails_with_invalid_role_id()
    {
        $response = $this->actingAs($this->user)->postGraphQL([
            'query' => '
                query GetRole($id: ID!) {
        ...
        $response->assertGraphQLErrorMessage('Role not found');
    }
    ...
    public function it_requires_authorization()
    {
        ...
        Gate::shouldReceive('denies')->with('view-role', $this->user)->andReturn(true);
    ...
    {
        RateLimiter::hit('role-query:' . $this->user->id, 5);
    ...
    {
        Log::shouldReceive('info')->once()->with('Role queried', ['user_id' => $this->user->id, 'role_id' => $this->role->id]);
